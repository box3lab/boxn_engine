...
import { Color3, MeshBuilder, PhysicsAggregate, PhysicsShapeSphere, StandardMaterial, Vector2, Vector3 } from "@babylonjs/core";
...
export class PlayerController extends BaseController {
    ...
    public initAnimator(): void {
        ...
        this.attack1State = this.playerEntity.animatorComponent?.addAnimationState("attack1", "Punch_1",1,false);
        this.magicAtkState = this.playerEntity.animatorComponent?.addAnimationState("magicAtk", "MagicAtk_1",1,false);
    }
    ...
    private createMagBullet(): void {
        const forward = this.playerEntity.skeletonMeshComponent?.meshRoot?.forward;
        if (forward) {
            const ball = MeshBuilder.CreateSphere("ball", { diameter: 0.2 }, this.playerEntity.scene?.scene);
            ball.position = this.playerEntity.root.root.position.clone().add(
                forward.scale(0.7)
            );
            ball.position.y += 1.3;

            const material = new StandardMaterial("ballMaterial", this.playerEntity.scene?.scene);
            material.diffuseColor = new Color3(0.5, 0.5, 0.5);
            ball.material = material;

            const shape = new PhysicsShapeSphere( new Vector3(0, 0, 0), 0.2, this.playerEntity.scene?.scene!);
            
            const physicsAggregate = new PhysicsAggregate(ball, shape, {
                mass: 10,
                restitution: 0.5,
                friction: 0.7
            }, this.playerEntity.scene?.scene);

            physicsAggregate.body.applyImpulse(forward.scale(500), ball.position);
        }
    }
